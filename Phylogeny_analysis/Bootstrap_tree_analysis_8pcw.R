...
for(i in 1:length(bootstrap_tree_files)) {
  ...
  tree_lost_clades=tree_node_numbers[which(!tree_node_numbers%in%shared_clades)]
...
data.frame(node=tree_node_numbers,retained=retained_node/length(boot_stats_list$nnodes_boot))%>%
...
plot(comb_plot2)
ggsave(comb_plot2,filename = "~/Documents/Foetal_paper_revisions/Bootstrap_trees_clade_robustness_8pcw.pdf",width = 7,height=7)
