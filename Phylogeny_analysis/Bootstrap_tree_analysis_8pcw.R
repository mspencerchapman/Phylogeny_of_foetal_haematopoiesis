...
plot(comb_plots)
ggsave(comb_plots,filename = "~/Documents/Foetal_paper_revisions/Bootstrap_score_comparisons_8pcw.pdf",device=cairo_pdf,width = 7,height=2)
...
plot(comb_plot1)
ggsave(comb_plot1,filename = "~/Documents/Foetal_paper_revisions/bootstrap_metrics_8pcw.pdf",width=7,height=2,units="in")
...
data.frame(node=tree_node_numbers,type="read_bootstraps",retained=retained_node_read_bootstraps)%>%
...
cairo_pdf(filename = "~/Documents/Foetal_paper_revisions/Low_confidence_split_8pcw.pdf",width = 15,height=8)
tree=plot_tree(tree,cex.label=0)
sapply(low_confidence_nodes$node, function(node) {
  info=get_edge_info(tree,details=NULL,node)
  draw.circle(x=info$x,y=info$yb,radius=1,col = "red")
})
dev.off()
...
plot(comb_plot2)
ggsave(comb_plot2,filename = "~/Documents/Foetal_paper_revisions/Bootstrap_trees_clade_robustness_8pcw.pdf",width = 7,height=7,device=cairo_pdf)
