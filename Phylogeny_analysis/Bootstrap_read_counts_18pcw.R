...
my_working_directory="/lustre/scratch119/casm/team154pc/ms56/fetal_HSC/fetal_18wks/tree_bootstraps"
...
sapply(R_function_files[c(1,2)],source)
...
for(i in 1:nsim) {
  ...
  Run_ID = paste0("Bootstrap_18pcw_",ids::random_id(1,8))
  dna_string_file = paste0(my_working_directory,"/mpboot_files/DNA_string_file_",Run_ID,".fa")
  mpboot_tree_file = paste0(dna_string_file,".treefile")
  tree_file_path = paste0(my_working_directory,"/final_trees/Tree_",Run_ID,".tree")
  
  #Load the reduced mats and params file from previous filtering
  load("/lustre/scratch119/casm/team154pc/ms56/fetal_HSC/filtering_runs/mats_and_params/mats_and_params_a1_MS2_both_reduced")
  load("/lustre/scratch119/casm/team154pc/ms56/fetal_HSC/Phylogeny_of_foetal_haematopoiesis/Data/18pcw/early_somatic_muts")
  ...
  output = remove_low_coverage_samples(COMB_mats = COMB_mats,
                                         ...
                                         other_samples_to_remove = "PD41768id_hum",
  ...
  COMB_mats=list_subset(COMB_mats,select_vector = (filter_params$germline_pval<0.001 & filter_params$bb_rhoval>0.1)|rownames(filter_params)%in%early_somatic_muts) #Pre-apply the beta-binomial filter, as boot-strapping
  ...
  XY_low_depth_cutoff = 4; XY_high_depth_cutoff = 9; AUTO_low_depth_cutoff = 8; AUTO_high_depth_cutoff = 18 #This is the same as used for the data
